...
for (threshold in c(5e-8, 1e-5)) {
  ...
  rownames(df) <- df$gwas
  numVar.df <- df
  save(numVar.df, file = paste("gwas.num_sig_vars.", as.character(threshold), ".nonmbsv3utr.RData", sep = ""))
  write.table(numVar.df, file = paste("gwas.num_sig_vars.", as.character(threshold), ".nonmbsv3utr.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
}
